...
export default function AppLayout({ children }) {
  ...
  const sidebarBg =
    theme === "dark"
      ? "bg-zinc-950 border-r border-zinc-800 text-gray-100"
      : "bg-white border-r border-gray-200 text-gray-800";

  const headingText = theme === "dark" ? "text-gray-100" : "text-gray-900";

  return (
    <div className={`h-screen flex ${bgGradient}`}>
      {/* Sidebar */}
      <aside className={`w-64 p-6 space-y-4 ${sidebarBg}`}>
        {!projectId ? (
          <>
          ...
          <>
            <h2 className={`text-xl font-semibold ${headingText}`}>Project Menu</h2>
            <NavLink href={`/app/project/${projectId}`} theme={theme}>Overview</NavLink>
            <NavLink href={`/app/project/${projectId}/boards`} theme={theme}>Boards</NavLink>
            <NavLink href={`/app/project/${projectId}/issues`} theme={theme}>Issues</NavLink>
            <NavLink href={`/app/project/${projectId}/sprints`} theme={theme}>Sprints</NavLink>
            <NavLink href={`/app/project/${projectId}/team`} theme={theme}>Team Members</NavLink>
            <NavLink href={`/app/project/${projectId}/analytics`} theme={theme}>Analytics</NavLink>
            <NavLink
              href="/app"
              theme={theme}
...
function NavLink({ href, children, className = "", theme }) {
  const linkHover =
    theme === "dark" ? "hover:text-white" : "hover:text-purple-700";

  return (
    <Link
      href={href}
      className={`block text-base transition-colors ${linkHover} ${className}`}
    >
      {children}
    </Link>
  );
